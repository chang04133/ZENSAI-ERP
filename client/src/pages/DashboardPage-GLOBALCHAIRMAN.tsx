...
export default function DashboardPage() {
  ...
  const moreToggle = (key: string, totalCount: number) => {
    if (totalCount <= VISIBLE_LIMIT) return null;
    const expanded = expandedSections[key];
    return (
      <div onClick={() => toggleSection(key)}
        style={{ textAlign: 'center', padding: '6px 0', cursor: 'pointer', fontSize: 12, color: '#1677ff', userSelect: 'none' }}>
        {expanded ? '접기 ▲' : `기타 ${totalCount - VISIBLE_LIMIT}개 더보기 ▼`}
      </div>
    );
  };
  const sliceData = <T,>(data: T[], key: string) => expandedSections[key] ? data : data.slice(0, VISIBLE_LIMIT);
            ...
            >
              {catStats.length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {sliceData(catStats, 'catStats').map((cat: any) => {
                  ...
                  {moreToggle('catStats', catStats.length)}
                </div>
              ) : (
            ...
            >
              {(prodDashboard.progressItems || []).length > 0 ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                  {sliceData(prodDashboard.progressItems || [], 'progress').map((item: any) => {
                  ...
                  {moreToggle('progress', (prodDashboard.progressItems || []).length)}
                </div>
              ) : (
          ...
          <Col span={24}>
            ...
            >
              <Table
                ...
                dataSource={sliceData(notifications, 'noti')}
                rowKey="notification_id"
                pagination={false}
                size="small"
                scroll={{ x: 750 }}
              />
              {moreToggle('noti', notifications.length)}
            </Card>
          </Col>
        </Row>
      )}
      ...
      <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
          ...
          {isAdmin && restockSuggestions.length > 0 ? (() => {
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      ...
                      {moreToggle('restockTop', topItems.length)}
                    </div>
                  );
                })()}
              </Card>
            ...
            <Card title={<span>{isStore ? '내 매장 재입고 필요' : '재입고 필요'} <Badge count={(stats?.lowStock || []).length} style={{ backgroundColor: '#ef4444', marginLeft: 8 }} /></span>}
              ...
              {(stats?.lowStock || []).length > 0 ? (
                <>
                <Table columns={lowStockColumns} dataSource={sliceData(stats?.lowStock || [], 'lowStock')} rowKey={(r: any) => `${r.partner_code}-${r.variant_id}`} pagination={false} size="small" scroll={{ x: 500 }} />
                {moreToggle('lowStock', (stats?.lowStock || []).length)}
                </>
              ) : (
        ...
        <Col xs={24} md={9}>
          <Card title={isStore ? '내 매장 최근 출고의뢰' : '최근 출고의뢰'} size="small" style={{ borderRadius: 10 }} loading={loading}
            extra={<a onClick={() => navigate(isStore ? '/shipment/view' : '/shipment/request')}>전체보기</a>}>
            <Table columns={shipmentColumns} dataSource={sliceData(stats?.recentShipments || [], 'shipments')} rowKey="request_no" pagination={false} size="small" scroll={{ x: 500 }}
              onRow={() => ({ onClick: () => navigate(isStore ? '/shipment/view' : '/shipment/request'), style: { cursor: 'pointer' } })} />
            {moreToggle('shipments', (stats?.recentShipments || []).length)}
          </Card>
        </Col>
        <Col xs={24} md={7}>
          <Card title={isStore ? '내 매장 인기상품 TOP 5' : '인기상품 TOP 5'} size="small" style={{ borderRadius: 10 }} loading={loading}
            ...
            {(stats?.topProducts || []).length > 0 ? (
              <>
              <Table columns={productColumns} dataSource={sliceData(stats?.topProducts || [], 'topProd')} rowKey="product_code" pagination={false} size="small" scroll={{ x: 400 }}
                onRow={() => ({ onClick: () => navigate('/sales/product-sales'), style: { cursor: 'pointer' } })} />
              {moreToggle('topProd', (stats?.topProducts || []).length)}
              </>
            ) : (
