...
function DashboardTab() {
  ...
  const [srchSubCategoryFilter, setSrchSubCategoryFilter] = useState('');
  const [srchYearFromFilter, setSrchYearFromFilter] = useState('');
  const [srchYearToFilter, setSrchYearToFilter] = useState('');
  const [srchSeasonFilter, setSrchSeasonFilter] = useState('');
  const [srchFitFilter, setSrchFitFilter] = useState('');
  const [srchColorFilter, setSrchColorFilter] = useState('');
  const [srchSizeFilter, setSrchSizeFilter] = useState('');
  const [srchSortValue, setSrchSortValue] = useState('qty_DESC');

  // Filter options (from codeApi, matching ProductListPage)
  const [categoryOptions, setCategoryOptions] = useState<{ label: string; value: string }[]>([]);
  const [allCategoryCodes, setAllCategoryCodes] = useState<any[]>([]);
  const [subCategoryOptions, setSubCategoryOptions] = useState<{ label: string; value: string }[]>([]);
  const [yearOptions, setYearOptions] = useState<{ label: string; value: string }[]>([]);
  const [seasonFilterOptions, setSeasonFilterOptions] = useState<{ label: string; value: string }[]>([]);
  ...
  // Inline results table state
  const [listData, setListData] = useState<any[]>([]);
  const [listTotal, setListTotal] = useState(0);
  const [listSumQty, setListSumQty] = useState(0);
  ...
  const [drillSearchSuggestions, setDrillSearchSuggestions] = useState<Array<{ product_code: string; product_name: string; category: string }>>([]);
  const drillSearchTimer = useRef<ReturnType<typeof setTimeout>>();

  const DRILL_VIEW_OPTIONS = [
    { label: '사이즈별 보기', value: 'size' },
    { label: '품번별 보기', value: 'product' },
    { label: '컬러별 보기', value: 'color' },
  ];
  const SORT_OPTIONS = [
  ...
  const loadDrill = useCallback(async (params: Record<string, string>, page: number, sort?: string, search?: string) => {
    setDrillLoading(true);
    try {
      const { sort_field, sort_dir } = parseDrillSort(sort || drillSort);
      const finalParams: Record<string, string> = { ...params, page: String(page), limit: '50', sort_field, sort_dir };
      const s = search ?? drillSearch;
      if (s.trim()) finalParams.search = s.trim();
      const result = await inventoryApi.list(finalParams);
      setDrillData(result.data);
      setDrillTotal(result.total);
      setDrillSumQty(result.sumQty);
    } catch (e: any) { message.error(e.message); }
    finally { setDrillLoading(false); }
  }, [drillSort, drillSearch]);
  ...
  const openDrillDown = useCallback((title: string, params: Record<string, string>) => {
    ...
    setDrillSearch('');
    setDrillSearchInput('');
    setDrillSearchSuggestions([]);
    loadDrill(params, 1, 'qty_desc', '');
  }, [loadDrill]);

  const onDrillSearchInput = (value: string) => {
    setDrillSearchInput(value);
    if (drillSearchTimer.current) clearTimeout(drillSearchTimer.current);
    if (!value.trim()) {
      setDrillSearchSuggestions([]);
      if (drillSearch) { setDrillSearch(''); setDrillPage(1); }
      return;
    }
    drillSearchTimer.current = setTimeout(async () => {
      try {
        const data = await inventoryApi.searchSuggest(value);
        setDrillSearchSuggestions(Array.isArray(data) ? data : []);
      } catch { setDrillSearchSuggestions([]); }
    }, 300);
  };

  const onDrillSearchSelect = (value: string) => {
    setDrillSearchInput(value);
    setDrillSearch(value);
    setDrillPage(1);
  };

  const onDrillSearchEnter = () => {
    if (drillSearchInput.trim()) {
      setDrillSearch(drillSearchInput.trim());
      setDrillPage(1);
    }
  };
  ...
  }, [drillPage, drillSort, drillSearch, loadDrill]);
  ...
  const onSearchChange = (value: string) => {
    setSearchText(value);
    if (debounceRef.current) clearTimeout(debounceRef.current);
    if (!value.trim()) { setSearchSuggestions([]); setListPage(1); loadList(1, ''); return; }
  ...
  const loadAll = useCallback(async () => {
    ...
    // Load filter options from codeApi (matching ProductListPage)
    ...
    codeApi.getByType('SEASON').then((data: any[]) => {
      setSeasonFilterOptions(data.filter((c: any) => c.is_active).map((c: any) => ({ label: c.code_label, value: c.code_value })));
    }).catch(() => {});
  ...
  // Category → sub-category cascading
  const handleCategoryFilterChange = (value: string) => {
    setSrchCategoryFilter(value);
    setSrchSubCategoryFilter('');
    setListPage(1);
  ...
  // Load inventory list with all filters
  const loadList = useCallback(async (pageOverride?: number, searchOverride?: string) => {
    const currentPage = pageOverride ?? listPage;
    setListLoading(true);
    try {
      const params: Record<string, string> = { page: String(currentPage), limit: '50' };
      const s = searchOverride !== undefined ? searchOverride : searchText;
      if (s.trim()) params.search = s.trim();
      if (srchCategoryFilter) params.category = srchCategoryFilter;
      if (srchSubCategoryFilter) params.sub_category = srchSubCategoryFilter;
      if (srchYearFromFilter) params.year_from = srchYearFromFilter;
      if (srchYearToFilter) params.year_to = srchYearToFilter;
      if (srchSeasonFilter) params.season = srchSeasonFilter;
      if (srchFitFilter) params.fit = srchFitFilter;
      if (srchColorFilter) params.color = srchColorFilter;
      if (srchSizeFilter) params.size = srchSizeFilter;
      const lastUnderscore = srchSortValue.lastIndexOf('_');
      params.sort_field = srchSortValue.substring(0, lastUnderscore);
      params.sort_dir = srchSortValue.substring(lastUnderscore + 1);
      if (effectiveStore) {
        // store users: auto-filter by their partner
      }
      const result = await inventoryApi.list(params);
      setListData(result.data);
      setListTotal(result.total);
      setListSumQty(result.sumQty);
    } catch (e: any) { message.error(e.message); }
    finally { setListLoading(false); }
  }, [listPage, searchText, srchCategoryFilter, srchSubCategoryFilter, srchYearFromFilter, srchYearToFilter, srchSeasonFilter, srchFitFilter, srchColorFilter, srchSizeFilter, srchSortValue, effectiveStore]);

  // Auto-load when filters change
  useEffect(() => {
    loadList();
  }, [listPage, srchCategoryFilter, srchSubCategoryFilter, srchYearFromFilter, srchYearToFilter, srchSeasonFilter, srchFitFilter, srchColorFilter, srchSizeFilter, srchSortValue]);
  ...
  const byColor = (stats?.byColor || []) as Array<{ color: string; product_count: number; variant_count: number; total_qty: number }>;
  const analyzeProduct = (productCode: string) => {
    setSearchText(productCode);
    setListPage(1);
    loadList(1, productCode);
  };
  ...
  const drillDisplayData = useMemo(() => {
    ...
    const sortGrouped = (arr: any[]) => {
      const qtyField = drillView === 'product' ? 'total_qty' : 'color_qty';
      return arr.sort((a, b) => {
        if (drillSort === 'qty_asc') return a[qtyField] - b[qtyField];
        if (drillSort === 'product_name_asc') return (a.product_name || '').localeCompare(b.product_name || '');
        if (drillSort === 'category_asc') return (a.category || '').localeCompare(b.category || '');
        if (drillSort === 'sku_asc') return (a.sku || '').localeCompare(b.sku || '');
        return b[qtyField] - a[qtyField]; // 기본: qty_desc
      });
    };
    ...
    if (drillView === 'product') {
      ...
      return sortGrouped(Object.values(map));
    }
    // color
    const map: Record<string, any> = {};
    drillData.forEach((r: any) => {
  ...
  return (
    <>
      ...
      {/* 검색 필터 (상품관리 UI 동일) */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginTop: 16, marginBottom: 16, alignItems: 'flex-end' }}>
        <div style={{ minWidth: 200, maxWidth: 320 }}><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>검색</div>
          <AutoComplete value={searchText} onChange={onSearchChange} onSelect={(v) => { setSearchText(v); setListPage(1); loadList(1, v); }}
            style={{ width: '100%' }}
            ...
            <Input placeholder="코드 또는 이름 검색" prefix={<SearchOutlined />}
              onPressEnter={() => { setListPage(1); loadList(1); }} />
          </AutoComplete></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>카테고리</div>
          <Select value={srchCategoryFilter} onChange={handleCategoryFilterChange} style={{ width: 120 }}
            options={[{ label: '전체 보기', value: '' }, ...categoryOptions]} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>세부</div>
          <Select value={srchSubCategoryFilter} onChange={(v) => { setSrchSubCategoryFilter(v); setListPage(1); }} style={{ width: 140 }}
            options={[{ label: '전체 보기', value: '' }, ...subCategoryOptions]} disabled={!srchCategoryFilter} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>연도(부터)</div>
          <Select allowClear value={srchYearFromFilter} onChange={(v) => { setSrchYearFromFilter(v || ''); setListPage(1); }} style={{ width: 90 }}
            placeholder="전체" options={yearOptions} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>연도(까지)</div>
          <Select allowClear value={srchYearToFilter} onChange={(v) => { setSrchYearToFilter(v || ''); setListPage(1); }} style={{ width: 90 }}
            placeholder="전체" options={yearOptions} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>시즌</div>
          <Select value={srchSeasonFilter} onChange={(v) => { setSrchSeasonFilter(v); setListPage(1); }} style={{ width: 110 }}
            options={[{ label: '전체', value: '' }, ...seasonFilterOptions]} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>핏</div>
          <Select value={srchFitFilter} onChange={(v) => { setSrchFitFilter(v); setListPage(1); }} style={{ width: 130 }}
            options={[{ label: '전체 보기', value: '' }, ...fitOptions]} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>색상</div>
          <Select showSearch optionFilterProp="label" value={srchColorFilter}
            onChange={(v) => { setSrchColorFilter(v); setListPage(1); }} style={{ width: 120 }}
            options={[{ label: '전체 보기', value: '' }, ...srchColorOpts]} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>사이즈</div>
          <Select showSearch optionFilterProp="label" value={srchSizeFilter}
            onChange={(v) => { setSrchSizeFilter(v); setListPage(1); }} style={{ width: 110 }}
            options={[{ label: '전체 보기', value: '' }, ...srchSizeOpts]} /></div>
        <div><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>정렬</div>
          <Select value={srchSortValue} onChange={(v) => { setSrchSortValue(v); setListPage(1); }} style={{ width: 150 }}
            options={[
              { label: '수량 많은순', value: 'qty_DESC' },
              { label: '수량 적은순', value: 'qty_ASC' },
              { label: '상품명순', value: 'product_name_ASC' },
        ...
        <Button onClick={() => { setListPage(1); loadList(1); }}>조회</Button>
      </div>

      {/* 재고 목록 테이블 */}
      <div style={{ marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <Tag color="blue" style={{ fontSize: 13 }}>{listTotal}건</Tag>
          <Tag style={{ fontSize: 13 }}>{listSumQty.toLocaleString()}개</Tag>
        </div>
        <Table
          columns={[
            { title: '거래처', dataIndex: 'partner_name', key: 'partner_name', width: 110 },
      ...
      {!effectiveStore && (
        <Row gutter={[16, 16]} style={{ marginTop: 16 }}>
          <Col xs={24}>
            <Card title={<span><BgColorsOutlined style={{ marginRight: 8 }} />색상별 재고현황</span>}
              size="small" style={{ borderRadius: 10 }} loading={statsLoading}>
              {byColor.length > 0 ? (
                <HBar data={byColor.map(c => ({ label: c.color, value: Number(c.total_qty), sub: `${c.product_count}상품 / ${c.variant_count}옵션` }))}
                  maxItems={15} onBarClick={(label) => openDrillDown(`색상: ${label}`, { color: label === '미지정' ? '' : label })} />
              ) : (
                <div style={{ textAlign: 'center', padding: 24, color: '#aaa' }}>0개</div>
              )}
            </Card>
          </Col>
        </Row>
      ...
      <Modal
        open={!!drillDown}
        onCancel={() => { setDrillDown(null); setDrillData([]); setDrillSearch(''); setDrillSearchInput(''); }}
        footer={null}
        width="95vw"
        style={{ top: 20, maxWidth: 1600 }}
        styles={{ body: { padding: '12px 16px', maxHeight: 'calc(100vh - 100px)', overflow: 'auto' } }}
        title={
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ fontSize: 18, fontWeight: 700 }}>{drillDown?.title}</span>
            <Tag color="blue" style={{ fontSize: 14 }}>{drillTotal}건</Tag>
            <Tag style={{ fontSize: 14 }}>{drillSumQty.toLocaleString()}개</Tag>
          </div>
        }
      >
        <div style={{ display: 'flex', gap: 12, marginBottom: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <AutoComplete
            ...
            onChange={onDrillSearchInput}
            onSelect={onDrillSearchSelect}
            options={drillSearchSuggestions.map(s => ({
              value: s.product_code,
              label: (
          ...
          >
            <Input
              placeholder="상품명, SKU, 상품코드 검색"
              prefix={<SearchOutlined />}
              onPressEnter={onDrillSearchEnter}
              allowClear
            />
          </AutoComplete>
          <div style={{ flex: 1 }} />
          <Select
            size="small"
            value={drillView}
            onChange={(v) => { setDrillView(v as 'size' | 'product' | 'color'); setDrillPage(1); }}
            style={{ width: 130 }}
...
function AdjustTab() {
  ...
  return (
    <>
      ...
      <Table columns={invColumns} dataSource={data} rowKey="inventory_id" loading={loading} size="small" sortDirections={['descend', 'ascend', 'descend']}
        scroll={{ x: 1100, y: 'calc(100vh - 240px)' }}
