...
export default function ProductionPlanPage() {
  ...
  const handleExcelTemplate = async () => {
    try {
      const res = await (await import('../../core/api.client')).apiFetch(productionApi.excelTemplateUrl);
      if (!res.ok) throw new Error('다운로드 실패');
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = '생산계획_양식.xlsx';
      a.click();
  ...
  const handleExcelImport = async (file: File) => {
    try {
      const result = await productionApi.excelImport(file);
      ...
      Modal.success({
        ...
        content: (
          <div>
            <p>생산계획이 생성되었습니다.</p>
            <p>• 품목: <strong>{summary.itemCount}건</strong></p>
            <p>• 총 수량: <strong>{summary.totalQty.toLocaleString()}개</strong></p>
            {summary.skipped?.length > 0 && (
              <div style={{ marginTop: 8, color: '#fa8c16', fontSize: 12 }}>
                <p>건너뛴 행:</p>
                {summary.skipped.map((s: string, i: number) => <div key={i}>- {s}</div>)}
              </div>
            )}
          </div>
        ),
  ...
  return (
    <div>
      ...
      <Card title={`생산계획 관리${statusFilter ? ` - ${STATUS_LABELS[statusFilter]}` : ''}`} extra={
        <Space>
          <Button icon={<DownloadOutlined />} onClick={handleExcelTemplate}>엑셀 양식</Button>
