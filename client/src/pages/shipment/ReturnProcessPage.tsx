...
export default function ReturnProcessPage() {
  ...
  const columns = [
    ...
    ...(!isStore ? [
      { title: '출발(반품처)', dataIndex: 'from_partner_name', key: 'from_partner_name', render: (v: string) => v || '-' },
    ] : []),
    { title: '도착(입고처)', dataIndex: 'to_partner_name', key: 'to_partner_name', render: (v: string) => v || '-' },
  ...
  return (
    <div>
      <PageHeader title="반품등록" extra={<Button type="primary" icon={<PlusOutlined />} onClick={() => { form.resetFields(); setItems([]); setModalOpen(true); }}>반품의뢰 등록</Button>} />
      ...
      <Modal title="반품의뢰 등록" open={modalOpen} onCancel={() => setModalOpen(false)} onOk={() => form.submit()} okText="등록" cancelText="취소" width={700}>
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          {!isStore && (
            <Form.Item name="from_partner" label="반품 출발 거래처" rules={[{ required: true, message: '거래처를 선택해주세요' }]}>
              <Select showSearch optionFilterProp="label" placeholder="거래처 선택" options={partnerOptions} />
            </Form.Item>
          )}
          <Form.Item name="to_partner" label={isStore ? '반품 보낼 곳' : '반품 입고 거래처'}>
