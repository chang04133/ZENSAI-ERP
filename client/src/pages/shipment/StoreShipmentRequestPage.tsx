...
export default function StoreShipmentRequestPage({ embedded }: { embedded?: boolean }) {
  ...
  const buildParams = useCallback(() => {
    const params: Record<string, string> = { request_type: '출고요청' };
  ...
  const handleCreate = async (values: any) => {
    ...
    const zeroQty = items.find((i) => !i.request_qty || i.request_qty <= 0);
    if (zeroQty) { message.error(`${zeroQty.product_name} (${zeroQty.size}): 수량을 입력해주세요`); return; }
    if (stockPartner) {
  ...
  const handleConfirmShipped = async () => {
    ...
    try {
      ...
      message.success('출고가 완료되었습니다. (본사 재고가 차감되었습니다)');
    ...
    return (
      <Table size="small" dataSource={detailItems} rowKey="variant_id" pagination={false}
        columns={[
          ...
          { title: '요청', dataIndex: 'request_qty', width: 70, align: 'right' as const },
  ...
  /* ══════════ 컬럼 정의 ══════════ */
  const baseColumns = [
    { title: '요청번호', dataIndex: 'request_no', key: 'request_no', width: 130 },
    { title: '요청일', dataIndex: 'request_date', key: 'request_date', width: 120,
      render: (v: string) => v ? new Date(v).toLocaleString('ko-KR', { month: 'numeric', day: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false }) : '-' },
    { title: '요청매장', dataIndex: 'to_partner_name', key: 'to_partner_name', width: 110, ellipsis: true,
      render: (v: string, r: any) => v ? <span>{v} <span style={{ color: '#999', fontSize: 11 }}>({r.to_partner})</span></span> : '-' },
    { title: '출고창고', dataIndex: 'from_partner_name', key: 'from_partner_name', width: 130, ellipsis: true,
    ...
    { title: '요청수량', dataIndex: 'total_request_qty', key: 'req_qty', width: 80, align: 'right' as const,
    ...
    if (st === 'PENDING') {
      return (
        <Space>
          ...
          {isAdmin && <Button size="small" type="primary" icon={<SendOutlined />} style={{ background: '#fa8c16', borderColor: '#fa8c16' }} onClick={() => handleOpenShippedModal(record)}>출고확인</Button>}
          {record.requested_by === user?.userId && (
    ...
    if (st === 'DISCREPANCY') {
      ...
      return (
        <Space>
          ...
          {isAdmin && (
            <Popconfirm title="수량 불일치를 확인하고 완료 처리하시겠습니까?" onConfirm={async () => {
              try { await shipmentApi.update(record.request_id, { status: 'RECEIVED' }); message.success('완료 처리되었습니다.'); loadCounts(); loadAll(allPage, statusFilter || undefined); }
              catch (e: any) { message.error(e.message); }
          ...
          {canCancel && <Popconfirm title="취소하면 재고가 복구됩니다. 취소하시겠습니까?" onConfirm={() => handleCancel(record.request_id)} okText="취소처리" cancelText="닫기"><Button size="small" danger icon={<CloseOutlined />}>취소</Button></Popconfirm>}
        </Space>
      );
    }
    if (st === 'REJECTED') {
  ...
  const actionColumn = { title: '', key: 'action', width: 240, render: renderActions };

  const allColumns = [...baseColumns.slice(0, 2), statusColumn, ...baseColumns.slice(2), actionColumn];
  const statusColumns = [...baseColumns, actionColumn];
  ...
  const openCreateModal = () => {
    ...
    if (hq) {
      form.setFieldsValue({ from_partner: hq.partner_code });
      loadStockForPartner(hq.partner_code);
    }
    setModalOpen(true);
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 16, alignItems: 'flex-end' }}>
        <div style={{ minWidth: 200, maxWidth: 320 }}><div style={{ fontSize: 11, color: '#888', marginBottom: 2 }}>검색</div>
          <Input placeholder="요청번호 검색" prefix={<SearchOutlined />} value={search}
      ...
      <Modal title="입고요청 등록" open={modalOpen} onCancel={() => setModalOpen(false)} onOk={() => form.submit()} confirmLoading={creating} okText="요청" cancelText="취소" width={700}>
        <Form form={form} layout="vertical" onFinish={handleCreate}>
          ...
          <Form.Item name="memo" label="메모"><Input.TextArea rows={2} placeholder="요청 사유를 입력하세요" /></Form.Item>
        </Form>
      </Modal>
