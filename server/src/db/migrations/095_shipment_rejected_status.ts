...
const m095: Migration = {
  version: 95,
  name: '095_shipment_rejected_status',
  async up(pool) {
    ...
    // REJECTED 포함한 새 CHECK 제약조건 추가
    await pool.query(`
      ALTER TABLE shipment_requests
      ADD CONSTRAINT shipment_requests_status_check
      CHECK (status IN ('PENDING', 'SHIPPED', 'RECEIVED', 'CANCELLED', 'DISCREPANCY', 'REJECTED'))
    `);
  },
};
