...
router.post('/', authMiddleware, requireRole('ADMIN', 'SYS_ADMIN'), asyncHandler(async (req, res) => {
  ...
  try {
    ...
    await audit('master_codes', String(result.rows[0].code_id), 'INSERT', req.user!.userId, null, result.rows[0]);
...
router.put('/:id', authMiddleware, requireRole('ADMIN', 'SYS_ADMIN'), asyncHandler(async (req, res) => {
  ...
  if (!code_value || !code_label) { res.status(400).json({ success: false, error: '코드값, 코드명은 필수입니다.' }); return; }
  const pool = getPool();
  const old = await pool.query('SELECT * FROM master_codes WHERE code_id = $1', [id]);
  if (old.rows.length === 0) { res.status(404).json({ success: false, error: '코드를 찾을 수 없습니다.' }); return; }
  const result = await pool.query(
  ...
  await audit('master_codes', String(id), 'UPDATE', req.user!.userId, old.rows[0], result.rows[0]);
...
router.delete('/:id', authMiddleware, requireRole('ADMIN', 'SYS_ADMIN'), asyncHandler(async (req, res) => {
  ...
  await audit('master_codes', String(id), 'DELETE', req.user!.userId, old.rows[0], null);
