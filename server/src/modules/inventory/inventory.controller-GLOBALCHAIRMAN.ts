...
class InventoryController extends BaseController<Inventory> {
  ...
  dashboardStats = asyncHandler(async (req: Request, res: Response) => {
    ...
    const [overall, byCategory, bySeason, byFit, byLength, byYear, byColor] = await Promise.all([
      ...
      inventoryRepository.summaryByColor(partnerCode),
    ]);
    res.json({ success: true, data: { overall, byCategory, bySeason, byFit, byLength, byYear, byColor, isStore: !!partnerCode } });
  });
