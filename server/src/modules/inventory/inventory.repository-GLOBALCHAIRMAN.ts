...
export class InventoryRepository extends BaseRepository<Inventory> {
  ...
  async listWithDetails(options: any = {}) {
    const { page = 1, limit: rawLimit = 20, partner_code, search, category, sub_category, season, size, color, fit, length, year, year_from, year_to, stock_level, sort_field, sort_dir } = options;
  ...
  /** 컬러별 재고 요약 */
  async summaryByColor(partnerCode?: string) {
    const params: any[] = [];
    let pcJoin = '';
    if (partnerCode) { params.push(partnerCode); pcJoin = 'AND i.partner_code = $1'; }
    const sql = `
      SELECT COALESCE(pv.color, '미지정') AS color,
             COUNT(DISTINCT p.product_code) AS product_count,
             COUNT(DISTINCT pv.variant_id) AS variant_count,
             COALESCE(SUM(i.qty), 0)::int AS total_qty
      FROM products p
      ...
      LEFT JOIN inventory i ON pv.variant_id = i.variant_id ${pcJoin}
      WHERE p.is_active = TRUE AND pv.is_active = TRUE
      GROUP BY COALESCE(pv.color, '미지정')
      ORDER BY total_qty DESC`;
    return (await this.pool.query(sql, params)).rows;
  }
    ...
    const sql = `
      ...
      LEFT JOIN LATERAL (
        ...
        FROM (
          ...
          JOIN partners sp ON si.partner_code = sp.partner_code
