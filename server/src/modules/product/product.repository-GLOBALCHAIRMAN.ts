...
export class ProductRepository extends BaseRepository<Product> {
  ...
  async list(options: any = {}) {
    const { page = 1, limit = 20, color, size, year_from, year_to, orderBy, orderDir, issue, event } = options;
    ...
    let eventFilterSql = '';
    if (event === 'on') {
      eventFilterSql = ` AND p.event_price IS NOT NULL AND (p.event_end_date IS NULL OR p.event_end_date >= CURRENT_DATE)`;
    } else if (event === 'off') {
      eventFilterSql = ` AND (p.event_price IS NULL OR (p.event_end_date IS NOT NULL AND p.event_end_date < CURRENT_DATE))`;
    }

    const countSql = `SELECT COUNT(DISTINCT p.product_code) FROM ${this.table} p ${variantJoin} ${issueCountJoin} ${whereClause}${variantFilter}${issueFilter}${eventFilterSql}`;
    ...
    const dataSql = `
      ...
      ${whereClause}${variantFilter}${issueFilter}${eventFilterSql}
  ...
  async listEventProducts(options: any = {}) {
    const { page = 1, limit = 50, search, category, season, fit, sub_category, color, size, active, expired, year_from, year_to, orderBy, orderDir } = options;
    const offset = (page - 1) * limit;
    const params: any[] = [];
    let nextIdx = 1;
    let extraFilters = '';
    ...
    const orderCol = allowedOrder[orderBy] || 'p.product_name';
  ...
  async bulkUpdateEventPrices(
    updates: Array<{ product_code: string; event_price: number | null }>,
    ...
    try {
      ...
      for (const { product_code, event_price } of updates) {
        const result = await client.query(
          ...
          [event_price, storeArr, sd, ed, product_code],
        );
